...
import { Note, fetchNotes, addNoteAsync, updateNoteAsync, deleteNoteAsync } from "@/redux/slices/notesSlice";
...
const NotesBoard: React.FC = () => {
  ...
  const handleSaveNote = async (note: Partial<Note>) => {
    if (note.id) {
      ...
      try {
        const result = await dispatch(updateNoteAsync(note as Note));
  ...
  const handleDeleteNote = (id: string) => {
    confirmModalRef.current?.open({
      ...
      type: "danger",
      onConfirm: async () => {
        const result = await dispatch(deleteNoteAsync(id));
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-3">
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1">
            <button
              className={`p-1.5 rounded-md transition-colors ${viewMode === "grid" ? "bg-white shadow" : "text-gray-600"
                }`}
            ...
            <button
              className={`p-1.5 rounded-md transition-colors ${viewMode === "list" ? "bg-white shadow" : "text-gray-600"
                }`}
